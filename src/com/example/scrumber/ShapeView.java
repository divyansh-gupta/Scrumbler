 ...
 */
public class ShapeView extends SurfaceView {
	// ~ Fields ................................................................
	...
	// Event forwarders
	private final CoordinateRespectingMotionEventDispatcher onTouchDown = new CoordinateRespectingMotionEventDispatcher(
	    "onTouchDown");
	private final CoordinateRespectingMotionEventDispatcher onTouchMove = new CoordinateRespectingMotionEventDispatcher(
	    "onTouchMove");
	private final CoordinateRespectingMotionEventDispatcher onTouchUp = new CoordinateRespectingMotionEventDispatcher(
	    "onTouchUp");
	private static final EventDispatcher onKeyDown = new EventDispatcher(
	    "onKeyDown");

	private static final EventDispatcher onScaleGesture = new EventDispatcher(
	    "onScaleGesture");
	private static final EventDispatcher onRotateGestureBegin = new EventDispatcher(
	    "onRotateGestureBegin");
	private static final EventDispatcher onRotateGesture = new EventDispatcher(
	    "onRotateGesture");
	private static final EventDispatcher onFlingGesture = new EventDispatcher(
	    "onFlingGesture");
	...
	private void init() {
		...
		backgroundColor = Color.fromRawColor(array.getColor(0,
	 ...
	 *            horizontal and vertical acceleration due to gravity (in
	 *            units/sec^2) of the physical world represented by this shape
	 *            view
	 */
	 ...
	 *            units/sec^2)
	 * @param yGravity the vertical acceleration due to gravity (in units/sec^2)
	 */
	 ...
	 *
	 * @param value Whether or not this view should automatically repaint when
	 *            shapes change.
	 */
	 ...
	 *            all classes).
	 * @param <MyShape> The type of shape to look for, as specified in the cls
	 *            parameter.
	 * @return A set of shapes that lie within the given circle.
	 */
	public <MyShape extends Shape> Set<MyShape> getShapesInRange(float x,
	    float y, float r, Class<MyShape> cls) {
	 ...
	 */
	public <MyShape extends Shape> Set<MyShape> getNeighbors(Shape shape,
	    float distance, boolean diag, Class<MyShape> cls) {
		if (distance < 0.0) {
	 ...
	 */
	public <MyShape extends Shape> Set<MyShape> getShapesInDirection(float x,
	    float y, float angle, float length, Class<MyShape> cls) {
		return null;
	...
	private void processTouchEvent(MotionEvent e, EventDispatcher event) {
		...
		if ((event == onTouchMove || event == onTouchUp)
			...
			PointF otherPt = coordinateSystem.deviceToLocal(e.getX() + 10,
			    e.getY());
	...
	private class CoordinateRespectingMotionEventDispatcher extends
			...
			if (xyTransformer == null) {
				xyTransformer = new MethodTransformer(float.class, float.class) {
					...
					protected Object[] transform(Object... args) {
						...
						PointF pt = coordinateSystem.deviceToLocal(e.getX(),
						    e.getY());
